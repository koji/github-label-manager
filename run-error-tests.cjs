...
const testScenarios = [
    ...
    },
    {
        ...
        requirement: '2.3',
        description: 'Should display format validation error when JSON is not an array',
...
function testNonExistentFile(filePath) {
    ...
    if (!fs.existsSync(filePath)) {
        ...
        return {
            success: true,
            message: 'Correctly handled non-existent file with user-friendly error message'
        };
    }
    return {
        success: false,
...
function testInvalidJsonSyntax(filePath) {
    ...
    try {
        ...
        try {
            ...
            return { success: false, message: 'JSON should be invalid for this test' };
        } catch (parseError) {
            log(colorize('red', `Error: Invalid JSON syntax in file: ${filePath}`));
            log(colorize('red', `Parse error: ${parseError.message}`));
            return {
                success: true,
                message: 'Correctly handled JSON parse error with detailed user-friendly message'
            };
        }
    } catch (error) {
...
function testInvalidDataStructure(filePath) {
    ...
    try {
        ...
        if (!Array.isArray(jsonData)) {
            log(colorize('red', 'Error: JSON file must contain an array of label objects'));
            return {
                success: true,
                message: 'Correctly handled non-array JSON structure with clear error message'
            };
        }

        return { success: false, message: 'JSON should not be an array for this test' };
    } catch (error) {
...
function testMissingRequiredFields(filePath) {
    ...
    try {
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (!item.name) {
                log(colorize('red', `Error: Item at index ${i} is missing required 'name' field`));
                hasValidationErrors = true;
                continue;
            }
            if (typeof item.name !== 'string') {
                log(colorize('red', `Error: Item at index ${i} has invalid 'name' field (must be a non-empty string)`));
                hasValidationErrors = true;
                continue;
            }
            if (item.name.trim() === '') {
                log(colorize('red', `Error: Item at index ${i} has empty 'name' field (name cannot be empty)`));
                hasValidationErrors = true;
                continue;
            }
        ...
        if (hasValidationErrors) {
            ...
            return {
                success: true,
                message: 'Correctly handled missing required fields with specific, user-friendly error messages'
            };
        }

        return { success: false, message: 'Should have validation errors for this test' };
    } catch (error) {
        return { success: false, message: `Unexpected error: ${error.message}` };
    }
}
    ...
    try {
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (labelObj.color !== undefined && typeof labelObj.color !== 'string') {
                log(colorize('red', `Error: Item at index ${i} has invalid 'color' field (must be a string)`));
                hasValidationErrors = true;
                continue;
            }

            // Validate description field type
            if (labelObj.description !== undefined && typeof labelObj.description !== 'string') {
                log(colorize('red', `Error: Item at index ${i} has invalid 'description' field (must be a string)`));
                hasValidationErrors = true;
                continue;
            }
            ...
            if (labelObj.color !== undefined && labelObj.color.trim() === '') {
                log(colorize('red', `Error: Item at index ${i} has empty 'color' field (color cannot be empty if provided)`));
                hasValidationErrors = true;
                continue;
            }
        ...
        if (hasValidationErrors) {
            ...
            return {
                success: true,
                message: 'Correctly handled invalid field types with specific, user-friendly error messages'
            };
        }
...
async function runErrorHandlingTests() {
    log(colorize('blue', '🧪 Testing Error Handling Scenarios for JSON Label Import'));
    log(colorize('blue', '='.repeat(60)));
    log(colorize('gray', 'Testing all sub-tasks from task 8: Test error handling scenarios'));
    log('');
    ...
    for (let i = 0; i < testScenarios.length; i++) {
        ...
        log(colorize('yellow', 'Expected behavior: Error should be handled gracefully with user-friendly message'));
    ...
    log('□ Appropriate error types are handled (file not found, JSON parse, validation)');
