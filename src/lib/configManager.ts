...
export class ConfigManager {
    ...
    constructor() {
        ...
        this.fallbackConfigPath = join(homedir(), '.github-label-manager-config.json');
    }
     ...
     */
    private async loadConfigFromPath(configPath: string): Promise<StoredConfigType | null> {
        try {
     ...
     */
    private async handleConfigLoadError(error: unknown, configPath: string, locationName: string): Promise<void> {
        if (error instanceof ConfigError) {
            switch (error.type) {
                case ConfigErrorType.CORRUPTED_FILE:
                    console.warn(`⚠️  Configuration file at ${locationName} location is corrupted: ${error.message}`);
                    console.warn(`   File: ${configPath}`);
                    console.warn(`   The file will be ignored and you'll be prompted for credentials.`);
                ...
                case ConfigErrorType.PERMISSION_DENIED:
                    console.warn(`⚠️  Permission denied accessing configuration file at ${locationName} location.`);
                    console.warn(`   File: ${configPath}`);
                    console.warn(`   Please check file permissions or run with appropriate privileges.`);
                    break;

                case ConfigErrorType.INVALID_FORMAT:
                    console.warn(`⚠️  Configuration file at ${locationName} location has invalid format.`);
                    console.warn(`   File: ${configPath}`);
                    console.warn(`   The file will be ignored and you'll be prompted for credentials.`);
                ...
                default:
                    console.warn(`⚠️  Failed to load configuration from ${locationName} location: ${error.message}`);
                    console.warn(`   File: ${configPath}`);
            }
        } else {
            console.warn(`⚠️  Unexpected error loading configuration from ${locationName} location.`);
            console.warn(`   File: ${configPath}`);
        }
    }
    ...
    private async backupCorruptedFile(configPath: string): Promise<void> {
        try {
            ...
            console.warn(`   Could not backup corrupted file: ${backupError instanceof Error ? backupError.message : 'Unknown error'}`);
        }
    }
    ...
    async saveConfig(config: StoredConfigType): Promise<void> {
        ...
        try {
            ...
            if (nodeError.code === 'EACCES' || nodeError.code === 'EPERM') {
                console.warn(`⚠️  Permission denied writing to primary configuration location.`);
                ...
                console.warn(`⚠️  Failed to save configuration to primary location: ${nodeError.message}`);
                console.warn(`   Trying fallback location...`);
            }

            // Try fallback location
            try {
                await fs.writeFile(this.fallbackConfigPath, configJson, { mode: 0o600 });
                console.warn(`✓ Configuration saved to fallback location: ${this.fallbackConfigPath}`);
                return;
            } catch (fallbackError) {
     ...
     */
    async validateCredentials(config: StoredConfigType): Promise<{ isValid: boolean; error?: ConfigError }> {
        try {
    ...
    async migrateToEncrypted(): Promise<void> {
        ...
        try {
            ...
            console.warn('⚠️  Failed to encrypt existing configuration:', error instanceof Error ? error.message : 'Unknown error');
        }
    }
     ...
     */
    async loadValidatedConfig(): Promise<{ config: StoredConfigType | null; shouldPromptForCredentials: boolean; preservedData?: Partial<StoredConfigType> }> {
        const config = await this.loadConfig();
        ...
        if (validation.error) {
            ...
            if (validation.error.type === ConfigErrorType.INVALID_FORMAT &&
                !validation.error.message.includes('Token belongs to user')) {
                preservedData.owner = config.owner;
                console.warn(`   Your GitHub username '${config.owner}' will be preserved.`);
            }
        }
        ...
        return {
            ...
            preservedData: Object.keys(preservedData).length > 0 ? preservedData : undefined
        };
    }
    ...
    async clearConfig(): Promise<void> {
        ...
        if (await this.fileExists(this.configPath)) {
            try {
                ...
                const nodeError = error as NodeJS.ErrnoException;
                if (nodeError.code === 'EACCES' || nodeError.code === 'EPERM') {
                    errors.push(`Permission denied removing primary config file: ${this.configPath}`);
                } else {
                    errors.push(`Failed to remove primary config file: ${nodeError.message}`);
                }
            }
        }
        ...
        if (await this.fileExists(this.fallbackConfigPath)) {
            try {
                ...
                const nodeError = error as NodeJS.ErrnoException;
                if (nodeError.code === 'EACCES' || nodeError.code === 'EPERM') {
                    errors.push(`Permission denied removing fallback config file: ${this.fallbackConfigPath}`);
                } else {
                    errors.push(`Failed to remove fallback config file: ${nodeError.message}`);
                }
            }
        }
