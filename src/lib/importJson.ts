...
export const importLabelsFromJson = async (
    ...
    try {
        ...
        try {
            ...
            log(chalk.red(`Parse error: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`));
            return;
        }
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (!labelObj.name) {
                log(chalk.red(`Error: Item at index ${i} is missing required 'name' field`));
                continue;
            }
            if (typeof labelObj.name !== 'string') {
                log(chalk.red(`Error: Item at index ${i} has invalid 'name' field (must be a non-empty string)`));
                continue;
            }
            if (labelObj.name.trim() === '') {
                log(chalk.red(`Error: Item at index ${i} has empty 'name' field (name cannot be empty)`));
                continue;
            }
            ...
            if (labelObj.color !== undefined) {
                if (typeof labelObj.color !== 'string') {
                    log(chalk.red(`Error: Item at index ${i} has invalid 'color' field (must be a string)`));
                    continue;
                }
                if (labelObj.color.trim() === '') {
                    log(chalk.red(`Error: Item at index ${i} has empty 'color' field (color cannot be empty if provided)`));
                    continue;
                }
            }
            ...
            if (labelObj.description !== undefined) {
                if (typeof labelObj.description !== 'string') {
                    log(chalk.red(`Error: Item at index ${i} has invalid 'description' field (must be a string)`));
                    continue;
                }
            ...
            if (unknownFields.length > 0) {
                log(chalk.yellow(`Warning: Item at index ${i} contains unknown fields that will be ignored: ${unknownFields.join(', ')}`));
            }
        ...
        for (let i = 0; i < validLabels.length; i++) {
            ...
            try {
                ...
                errorCount++;
                log(chalk.red(`${progress} Failed to create label "${label.name}": ${error instanceof Error ? error.message : 'Unknown error'}`));
        ...
        log(''); // Empty line for better readability
        if (errorCount === 0) {
            log(chalk.green(`✅ Import completed successfully! Created ${successCount} labels.`));
        } else {
        ...
        log(chalk.red(`Error reading file: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
};