...
function testErrorScenarios() {
    ...
    const scenarios = [
        ...
        },
        {
            ...
            test: () => {
                try {
                    const content = fs.readFileSync('test-data/invalid-json-syntax.json', 'utf8');
                    JSON.parse(content);
        ...
        },
        {
            ...
            test: () => {
                try {
                    const content = fs.readFileSync('test-data/invalid-structure-not-array.json', 'utf8');
        ...
        },
        {
            ...
            test: () => {
                try {
                    const content = fs.readFileSync('test-data/missing-required-fields.json', 'utf8');
                    const data = JSON.parse(content);
        ...
        },
        {
            ...
            test: () => {
                try {
                    const content = fs.readFileSync('test-data/invalid-field-types.json', 'utf8');
                    const data = JSON.parse(content);
                    if (!Array.isArray(data)) return false;
                    ...
                    return data.some(item =>
                        ...
                        (item && item.color !== undefined && typeof item.color !== 'string') ||
                        (item && item.description !== undefined && typeof item.description !== 'string') ||
...
async function runComprehensiveTests() {
    ...
    if (overallSuccess) {
        ...
        log(colorize('blue', 'Error handling implementation meets all requirements:'));
        ...
        log(colorize('red', `❌ ${categories.length - passedCategories} test categories failed`));
    }
