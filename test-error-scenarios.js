...
async function runErrorHandlingTests() {
    ...
    for (let i = 0; i < testScenarios.length; i++) {
        ...
        log(chalk.yellow('Expected behavior: Error should be handled gracefully with user-friendly message'));
        ...
        try {
            ...
            const hasExpectedError = output.toLowerCase().includes(scenario.expectedError.toLowerCase());
...
async function testErrorMessageQuality() {
    ...
    const qualityTests = [
        ...
        },
        {
            ...
            test: async () => {
                startCapture();
                await importLabelsFromJson(mockConfigs, 'test-data/missing-required-fields.json');
                const output = stopCapture();
...
async function main() {
    try {
        ...
        if (allTestsPass) {
            ...
            log(chalk.blue('The implementation correctly handles all error scenarios with user-friendly messages.'));
        } else {
